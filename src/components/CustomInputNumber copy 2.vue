<template>
    <div type="button" class="custom-input-number flex justify-between items-center">
        ...
        <input class="grow max-w-[5ch]" v-on:change="check" type="number" name="" id="" v-model.number="inputValueRef">
...
const check = () => {
    if (typeof inputValueRef.value === "String") {
        console.log("do_something")
        inputValueRef.value = eval(inputValueRef.value)
        return
    }
}
