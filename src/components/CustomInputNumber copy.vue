<template>
    <div type="button" class="custom-input-number flex justify-between items-center">
        <span class="grow max-w-[5ch]" contenteditable="true" @keyup="updateValue" ref="inputValueRef">{{
            inputValue
        }}</span>
...
const updateValue = () => {
    inputValue.value = inputValueRef.value.innerText
}

watchEffect(() => {
    console.log(inputValue.value)
    /*     if (inputValueRef.value > 99) {
            inputValueRef.value = inputValueRef.value
        } */
})
